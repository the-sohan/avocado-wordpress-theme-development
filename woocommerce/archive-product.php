	...
	<div class="row">
		...
		<div class="<?php echo $main_col ?>">
			...
			?>	
				...
				<div class="row child-cat-list">
					<?php 
						foreach($childrens as $children) :
						$child_thumb_id = get_woocommerce_term_meta( $children, 'thumbnail_id', true );
						$child_thumb_uri = wp_get_attachment_image_url( $child_thumb_id, 'thumbnail' );
						$child_info = get_term( $children, 'product_cat' );
					?>
					<div class="col child_cat text-center my-auto">
						<a href="<?php echo get_term_link($children, 'product_cat'); ?>">
							<?php if(!empty($child_thumb_id)) : ?>
								<img src="<?php echo $child_thumb_uri; ?>" alt="">
							<?php endif; ?>
							<?php echo wpautop($child_info->name); ?>
						</a>	
					</div>
					<?php endforeach; ?>
				</div>
